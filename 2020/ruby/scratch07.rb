...
pp(begin
  data = I7.split("\n").map do |l|
    a = l.match(/\A([\w\s]+) bags contain no other bags./)
    next [a[1], {}] if a

    a = l.match(/\A([\w\s]+) bags contain ((?:\d+ [\w\s]+ bags?[,.] ?)+)\z/)[1..2]
    [a[0], a[1].split(", ").map{_1.match(/\A(\d+) ([^.]+) bags?\.?\z/)[1..2].reverse}.to_h.transform_values(&:to_i)]
  end
  ...
  @keys = ["shiny gold"]
  loop do
    break if @keys.length == @last_keys.length

    @last_keys = @keys
    @keys += data.select{ _1[1].keys.any? { |k| @keys.include? k } }.map(&:first)
    @keys.uniq!
  end

  p @keys.length - 1

  count_bags("shiny gold", data.to_h)


  [DATA[:members][:"401885"],
  DATA[:members][:"963596"]].map{[_1[:name],_1[:completion_day_level][:"5"].values.map(&:values).flatten.map(&:to_i).sort.map{|t|Time.at t}]}
