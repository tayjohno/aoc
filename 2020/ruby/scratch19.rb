...
# Rule types:
# type: %i[prefix suffix]
# a:
# b:

# matched and remainder both return char arrays
Match = Struct.new(:matched, :remainder)

class Rule
  attr_reader :paths

  def initialize(data)
    @paths = data.map do |path|
      path.map do |step|
        char = step.match(/\"([ab])\"/)&.[](1)
        next char if char
        step.to_i
      end
    end
  ...
  def char?
    @char ||= begin
      paths.length == 1 &&
        paths.first.length == 1 &&
        ["a", "b"].include?(paths.first.first)
    end
  end

  def matches(chars)
    raise "what is a #{chars}" unless chars.is_a?(Array) && chars.all? { |char| char.is_a?(String) }
    if char?
      [Match.new(chars[0], chars[1..-1])] if chars[0] == paths[0][0]
    else
      paths.map do |path|
        path.reduce([Match.new([],chars.dup)]) do |matches, step|
          nil if chars.nil?
          matches.map do |match|
            RULES[step].matches(match.remainder)
          end.flatten.compact
        end
      end.flatten
    end
...
tmp_rules, inputs = *I19.split("\n\n").map{_1.split("\n")}
INPUTS = inputs.map{_1.split("")}.freeze
RULES = tmp_rules.map do |rule_string|
  _,i,rest = *rule_string.match(/\A(\d+): (.*)\z/)
  raise "missing rule number #{rule_string}" unless i
  [i.to_i, Rule.new(rest.split("|").map{_1.split})]
end.to_h


pp(begin
  INPUTS.map do |input|
    RULES[0].matches(input)
  end.count do |match|
    match.any? {_1.remainder.empty?}
  end

  RULES.merge!({8 =>Rule.new([["42"],["42","8"]]), 11 => Rule.new([["42","31"],["42","11","31"]])})
