...
import Day16.Input exposing (..)
import Day16.Model exposing (..)
...
partOne _ =
    input1
        |> List.map (matchingOps >> List.length)
        |> List.filter ((<=) 3)
        |> List.length
        |> String.fromInt
        |> Solved
...
partTwo _ =
    input2
        |> List.foldl
            (\instr -> Result.andThen (exec instr))
            (Ok emptyRegisters)
        |> Result.map (.a >> String.fromInt >> Solved)
        |> Result.withDefault Unsolved


exec : Instruction -> Registers -> Result String Registers
exec instruction registers =
    let
        { op } =
            lookupOpCode instruction.opCode
    in
    op registers instruction.arguments
...
matchingOps : CPUSample -> List OpDefinition
matchingOps { startingState, endingState, instruction } =
    allOps
        |> List.filter (\{ op } -> op startingState instruction.arguments == Ok endingState)


lookupOpCode : Int -> OpDefinition
lookupOpCode int =
    allOps
        |> List.filter (.code >> (==) (Just int))
        |> (\ops ->
                case ops of
                    op :: [] ->
                        op
                    ...
                    _ ->
                        Debug.todo "Invalid opcode or configuration"
           )
...
When solving the op codes, these were used to manually determine their values.
--}


determineOpCodes =
    input1
        |> List.filter (\sample -> not (List.member sample.instruction.opCode solvedOpCodes))
        |> List.map
            (\sample ->
                ( sample.instruction.opCode
                , matchingUnsolvedOps sample
                )
            )
...
matchingUnsolvedOps : CPUSample -> List OpDefinition
matchingUnsolvedOps { startingState, endingState, instruction } =
    allOps
        |> List.filter (.code >> (==) Nothing)
        |> List.filter (\{ op } -> op startingState instruction.arguments == Ok endingState)
...
validateOpCodes =
    input1
        |> List.filter
            (\{ startingState, endingState, instruction } ->
                (lookupOpCode instruction.opCode |> .op) startingState instruction.arguments /= Ok endingState
            )
        |> List.map (\{ instruction } -> instruction.opCode)
